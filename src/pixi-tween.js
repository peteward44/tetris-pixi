...
var Easing = {
	...
	inElastic: function inElastic() {
		...
		let p = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0.4;
	...
	inBack: function inBack() {
		let v = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1.70158;
...
let Tween = (function(_PIXI$utils$EventEmit) {
	...
	function Tween(target, manager, config) {
		...
		let _this = possibleConstructorReturn(this, (Tween.__proto__ || Object.getPrototypeOf(Tween)).call(this));
	...
	createClass(Tween, [{
		...
		value: function _end() {
			...
			if (this._chainTween) {
				...
				let resolvePromise = this._resolvePromise;
		...
		value: function _apply(time) {
			...
			if (this.path) {
				let _time = this.pingPong ? this.time / 2 : this.time;
				let b = this._pathFrom;
				let c = this._pathTo - this._pathFrom;
				let d = _time;
				let t = _time ? this._elapsedTime / d : 1;
	...
	createClass(TweenManager, [{
		...
		value: function update(deltaMS) {
			...
			if (this._tweensToDelete.length) {
				for (let _i = 0; _i < this._tweensToDelete.length; ++_i) {
...
let TweenPath = (function() {
	...
	createClass(TweenPath, [{
		...
		value: function getPointAt(num) {
			...
			let _getPoint = this.getPoint(Math.ceil(num)),
				ceilX = _getPoint.x,
				ceilY = _getPoint.y;

			let _getPoint2 = this.getPoint(Math.floor(num)),
				floorX = _getPoint2.x,
				floorY = _getPoint2.y;
		...
		value: function distanceBetween(num1, num2) {
			...
			let _getPoint3 = this.getPoint(num1),
				p1X = _getPoint3.x,
				p1Y = _getPoint3.y;

			let _getPoint4 = this.getPoint(num2),
				p2X = _getPoint4.x,
				p2Y = _getPoint4.y;
